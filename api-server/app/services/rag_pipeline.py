...
class RAGPipeline:
    ...
    def process_pdf(self) -> Dict[str, any]:
        ...
        try:
            ...
            # Step 1: Find and extract PDF
            logger.info("Step 1: Extracting PDF text")
            try:
                pdf_path = self.pdf_processor.find_pdf()
            except FileNotFoundError as e:
                raise Exception(f"PDF not found: {e}")
            ...
            for doc in documents:
                ...
                for chunk_idx, chunk_text in enumerate(split_texts):
                    chunks.append({
                        "text": chunk_text,
                        "title": f"page_{page_number:04d}_chunk_{chunk_idx}",
            ...
            logger.info(f"Created {total_chunks} chunks from {len(documents)} pages")
            
            # Step 3: Clear existing vector database
            logger.info("Step 3: Clearing existing vector database")
            self.vector_db.clear()
            
            # Step 4: Add chunks to vector database
            logger.info("Step 4: Creating embeddings and adding to vector database")
            chunks_added = self.vector_db.add_documents(chunks)
            
            logger.info(f"Successfully added {chunks_added} chunks to vector database")
            ...
            return {
                ...
                "message": f"Successfully processed PDF and created {chunks_added} chunks"
            }
            ...
            return {
                "status": "failed",
                "error": str(e),
                "message": f"RAG pipeline failed: {e}"
            }
